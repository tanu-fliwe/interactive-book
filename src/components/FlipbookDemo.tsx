...
const HTMLFlipBook = dynamic(async () => (await import("react-pageflip")).default, {
  ssr: false,
});
...
// react-pageflip expects each page child to accept a ref
const Page = React.forwardRef<HTMLDivElement, PageProps>(function Page(
  ...
  return (
    <div ref={ref} className="relative h-full w-full bg-white">
      {/* Placeholder page background (swap for your exported PNG later) */}
      ...
      <div className="relative p-6">
        ...
        <div className="mt-2 text-2xl font-extrabold text-slate-900">Page {pageNumber}</div>
        <div className="mt-2 max-w-md text-sm text-slate-700">
          Placeholder page background. Next step: swap this with your real exported page PNG.
        </div>
      </div>
